...
def search_params_v5(df_train, df_val, target_cols, exog_cols = None, max_lags = 5, max_neighbour_lags = 3, rollings = None, n_trials = 50):
    y_train = df_train[target_cols]
    y_val = df_val[target_cols]
    X_train = None
    X_val = None
    ...
    def objective(trial):
        ...
        for i in range(0, len(target_cols)):
            ...
            if i != 0:
                neighbor_col = target_cols[i - 1]
                lags_dict[neighbor_col] = trial.suggest_int(f'neighbor_lags_{col}_up', 1, max_neighbour_lags)
            if i != len(target_cols) - 1:
                neighbor_col = target_cols[i + 1]
                lags_dict[neighbor_col] = trial.suggest_int(f'neighbor_lags_{col}_down', 1, max_neighbour_lags)
            models.append(ForecasterDirectMultiVariate(estimator=xgb.XGBRegressor(**param),
                                                   level = col,
                                                   steps = 1,
                                                   lags = lags_dict,
